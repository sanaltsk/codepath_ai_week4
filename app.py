...
You are a software developer, preparing to build the web page in the image that the user sends. 
From the plan.md file pick up the first task and and write html and css code required to implement the recommended 
task. 

If the user or reviewer confirms the code is good, available tools to save it as an artifact \
called `index.html` and `style.css`. Your role is to implement the plan and write specific code to \
accomplish the task.

Once done go back to the plan.md and update the task as completed and write the same to file. 
"""

# Create an instance of the Agent class
planning_agent = PlanningAgent(name="Planning Agent", client=client, prompt=PLANNING_PROMPT)
implementation_agent = ImplementationAgent(name="Implementaion Agent", client=client, prompt=IMPLEMENTATION_PROMPT)
